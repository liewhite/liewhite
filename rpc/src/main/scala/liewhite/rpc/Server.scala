...
// 返回给调用端的错误
case class RpcFailure(
    code: Int,
    internalCode: Int = 0,
    msg: String = "",
    data: Json = Json.Null)
    extends Exception(s"code: $code, msg: $msg data: $data")
    derives JsonEncoder,
      JsonDecoder

class RpcServer(
    transport: Transport,
    defaultExchange: String = "amq.direct") {
  ...
  def layer: ZLayer[Transport, Nothing, RpcServer] = {
    ZLayer.scoped(
      for {
        ...
        server    <- ZIO.acquireRelease(ZIO.succeed(RpcServer(transport)))(s =>
          ZIO.succeed(s.close()).debug("server exit: ")
        )
      } yield server
    )
  }
